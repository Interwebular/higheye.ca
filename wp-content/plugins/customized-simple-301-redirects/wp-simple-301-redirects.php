<?php
/*
Plugin Name: Customized Simple 301 Redirects
...
if (!class_exists("Simple301redirects")) {
	...
	class Simple301Redirects {
		 ...
		 * @access public
		 * @return void
                 * edited this function to make redirect from www to non-www
		 */
		function redirect() {
			...
			if (!empty($redirects)) {
				...
				$rootdomain = ltrim(get_bloginfo('wpurl'), 'http://'); //added to handle www
				$len = strlen($rootdomain); //added to handle www
				...
				foreach ($redirects as $storedrequest => $destination) {
					...
					elseif(urldecode(substr($userrequest, $len+ 11)) == rtrim($storedrequest,'/')) { //added to handle www
						// simple comparison redirect
						$do_redirect = $destination;
					}
